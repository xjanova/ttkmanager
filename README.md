...
[![License](https://img.shields.io/badge/License-Commercial-a3e635?style=flat-square)](#license)
[![Status](https://img.shields.io/badge/Status-v0.1%20alpha-ef4444?style=flat-square)](#)

A product of **xman studio** · [xman4289.com](https://xman4289.com)
...
>
> A progress dialog walks through seeding three sample TikTok accounts, seven campaigns, six schedule rules, five auto-rules, four budget caps, fourteen days of metric samples (~1.2k rows), seven alerts, and thirty-two audit entries — all flagged in SQLite as demo records. Click **▶ Switch to Live** and every demo row is deleted; your real data is untouched.

## License & activation

TTK Manager uses the **xman studio license server** (the same activation system used by other xman studio products):

- Keys look like `TTK-XXXX-XXXX-XXXX`
- Activation locks a key to your machine fingerprint
- Offline grace: cached license is honoured when the server is unreachable
- Plans: **Free / Daily / Weekly / Monthly / Yearly / Lifetime**

Switching from Demo Mode to Live Mode opens the License dialog if no key is active.
...
| Operations | License activation + offline grace | ✅ |
...
└── src/TTKManager.App/
    ...
    │   ├── XmanLicenseService.cs    xman studio license activation
...
| License | xman studio license server (`xman4289.com/api/v1/products/ttkmanager`) |
...
```json
{
  ...
  "RedirectUri": "https://xman4289.com/ttkmanager/callback",
  "UseMockApi": false
}
```
...
3. After approving, TikTok redirects to the callback page with `?auth_code=…`
...
| Phase | Scope | When |
|-------|-------|------|
| **P1 Foundation** | Dashboard, Accounts, Campaigns, Schedules, Logs, Settings, Backup, Health, License | ✅ shipped |
...
Commercial product of xman studio. Activation required via `TTK-XXXX-XXXX-XXXX` keys issued at [xman4289.com](https://xman4289.com).
